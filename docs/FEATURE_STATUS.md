...
| FEAT-008 | 디자인·오버레이·텍스트 | DESIGN-001 | 기존 / 1.3.1 | 부분 완료 | Stage 4, `video_gen.py` | 연결 | 연결 | 연결 | 상태 복원·네 해상도 픽셀 비교 | 미적 품질 수동 검수 필요 |
...
| FEAT-012 | 라이브·두 곡 미리보기 | PREVIEW-001, PREVIEW-002 | 1.3.0 / 1.3.1 | 부분 완료 | Stage 4, `video_gen.py`, `audio_preview.py` | 연결 | 연결 | 해당 없음 | 공통 경로·네 해상도 픽셀 비교 | 실제 장시간 미디어 수동 검수 필요 |
...
| FEAT-022 | Windows·macOS 폴더형 배포 | BUILD-001 | 1.3.0 / 1.3.0 | 부분 완료 | build scripts, workflow | 해당 없음 | 양 플랫폼 빌드 연결 | 해당 없음 | Windows 실행 smoke·양 플랫폼 CI ZIP | macOS `.app` 실제 실행·종료는 ISSUE-BUILD-003 |
...
| FEAT-025 | 추가형 효과 카드·선택 메뉴 | EFFECT-002 | 1.3.0 / 1.3.1 | 완료 | Stage 4, `ui_state.py` | 연결 | 연결 | migration 연결 | 빈 상태·검색·키보드·reset·deep-copy·submenu UI 테스트 | 단일 effect 인스턴스, 카드 순서는 inspector 순서 |
| FEAT-026 | 단일 환경음 효과·독립 ambient bus | AUDIO-004 | 1.3.0 / 1.3.1 | 완료 | `ambient_library.py`, `ambient_engine.py`, `audio_pipeline.py`, Stage 4·5 | 단일 `ambience_mixer` 연결 | chunk/batch 중간 bus 연결 | legacy global ambient migration 연결 | 13종 동시 실제 FFmpeg, 최대 8-input, 반복 환경음, 안전 재검색 | 장시간 성능·미확인 라이선스 검토 필요 |
...
| FEAT-028 | 단일 EXE·로딩 스플래시 | UI-002, UI-006, PERF-001 | 1.3.0 / 1.3.1 | 완료 | `app.py`, build scripts, brand assets | 연결 | 연결 | 해당 없음 | 최종 main 64.536초·최대화/resize 가능·종료 후 프로세스 0 | PyInstaller cold-start 추가 최적화 필요 |
# 1.3.1 효과 편집 안정화

- 미리보기 오른쪽 고정 효과 랙/효과별 설정 창: 구현·자동 검증 완료
- 효과 활성화/삭제/경고/단일 인스턴스 저장 복원: 구현 완료
- 실제 album/logo overlay 순서 변경: 구현·픽셀 검증 완료
- visualizer 배경 보존: 픽셀 검증 완료
- trim·중복 파일 beat timeline: 수정·검증 완료
- preview audio 설정 갱신 상태 보존: 구현 완료
- 모든 DPI·장문 locale 수동 UI 검수: 미완료

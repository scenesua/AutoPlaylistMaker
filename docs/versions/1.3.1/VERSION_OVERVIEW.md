...
- ISSUE-PROJECT-001 프로젝트 이름 지정·저장 차단 회귀
- ISSUE-NAV-001 다음 Stage 이동 차단 회귀
- ISSUE-UI-004 테마 전환 뒤 UI 요소 소실 회귀
- ISSUE-ANALYSIS-001 분석 progress window 조기 종료 회귀
...
- 1.3.0 기능과 데이터 호환성을 유지하며 프로젝트 진행을 막는 릴리스 후 회귀부터 재현하고 수정한다.
- 프로젝트 이름 지정·저장·다음 단계, 양방향 테마 전환, 분석 진행 피드백을 end-to-end 테스트로 고정한다.
